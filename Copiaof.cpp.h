...
{
	///Imagens de fundo
	fundoMenu.street.loadImage("Menu/FundoMenu3.png");
	fundoMenu.posicao.x = 0;
	fundoMenu.posicao.y = 0;
	gameOver.street.loadImage("Menu/GameOver.png");
	gameOver.posicao.x = 0;
	gameOver.posicao.y = 0;
	fundo.posicao.x = 0;
	fundo.posicao.y = 0;
	...
	///Sons do jogo
	//Menu
	som.menuSound.loadSound("Sonorizacao/audioMenu.WAV");
	som.choiceGameplay.loadSound("Sonorizacao/Choice_Gameplay.WAV");
	som.choiceTutorial.loadSound("Sonorizacao/Choice_Tutorial.WAV", true);

	//Turorial 
	som.tutorialSound.loadSound("Sonorizacao/Tutorial.WAV");

	//Player
	som.shooting.loadSound("Sonorizacao/Boomerang_Player.WAV", true);
	som.enemyHit.loadSound("Sonorizacao/Enemy_Hit.WAV");
	som.enemyHit.setVolume(0.05f);
	som.dashPlayer.loadSound("Sonorizacao/Player_Dash.WAV");
	som.playerDie.loadSound("Sonorizacao/Player_Diyng.WAV");
	som.playerDie.setVolume(0.5f);
	//Boss e inimigos
	som.scream.loadSound("Sonorizacao/Enemy_Scream.WAV", true);
	...
	//definicoes do tiro
	ataque.sprite.loadImage("players/armaPlayer2.png");
	ataque.sprite2.loadImage("players/armaPlayerUp2.png");
...
void ofApp::update()
{
	...
	case Tutorial:
		...
		colisao(player, inimigoTutorial, som);
		colisaoTiro(inimigoTutorial, ataque, player, som);
	...
	case GamePlay:
		...
		if (player.vida > 0)
		{
			...
			for (int i = 0; i < Ninimigo; i++)
			{
				...
				inimigosInGame(inimigo[i], player, som, time);
			}

			//BOSS

			//Boss no jogo
			BossInGame(Boss, player, som, time);
...
void ofApp::draw()
{
	...
	case Menu:
		...
		fundoMenu.street.draw(fundoMenu.posicao);

		ofDrawBitmapString("Posicao seta: " + ofToString(menuSelecao.posicao), 10, 10);
		break;
	case Tutorial:
...
void ofApp::keyPressed(int key)
{
	...
	case Menu:
		...
		{
			estadoJogo = Tutorial;
			som.choiceTutorial.play();
			som.tutorialSound.setLoop(true);
			som.tutorialSound.play();
		}
		if (key == '2')
		{
			som.choiceGameplay.play();
			estadoJogo = GamePlay;
		}
		if (key == '3')
			break;
